...
### Fixed
- Copilot tmux bell hook now suppresses hook stderr, uses bash-only `ask_user`
  matching, and has a shorter timeout so pre-tool hooks fail open more reliably.
...
### Changed
- Git commit signing config is now environment-aware: `gitconfig` includes
  `~/.gitconfig.local` (generated by `install.sh`), which enables 1Password
  SSH signing on macOS and disables signing in Codespaces
